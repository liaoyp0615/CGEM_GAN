...
def load_check_real_data(datafile, num):
    ...
    while(i<num):
        ...
        if max(tmp)!=0:
            ...
            _x,_z = tmp.sum(axis=1), tmp.sum(axis=0)
            cut_x, cut_z,_sum = _x[1:91], _z[1:91], _x.sum()
...
def check_fake_data(num):
    ...
    for i in range(nevt):
        ...
        fake_data = G(noise)
        fake_hist = fake_data.detach().cpu().numpy()
...
def plot_all_info():
    ...
    plt.legend()
    plt.subplot(3,3,3) #x
    plt.hist( F_x_Std,bins=300, range=(0,4), color = 'steelblue', histtype='step', label="fake data")
    plt.hist( R_x_Std,bins=300, range=(0,4), color = 'red', histtype='step', label="real data")
    plt.xlabel("x diff Std/mm")
    plt.legend()
    plt.subplot(3,3,4) #z
    plt.hist( F_z_Std,bins=300, range=(0,4), color = 'steelblue', histtype='step', label="fake data")
    plt.hist( R_z_Std,bins=300, range=(0,4), color = 'red', histtype='step', label="real data")
    plt.xlabel("z diff Std/mm")
    plt.legend()
    plt.subplot(3,3,5) #sum
...
def train_D(): # w-gan
    ...
    d_r_acc = torch.mean((d_r_score),0).detach().cpu().numpy()
    d_f_acc = torch.mean((d_f_score),0).detach().cpu().numpy()
...
def train_G():
    ...
    return g_loss.item(), torch.mean(g_score,0).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    Dataset_Train = HistoDataset(datafile,5000)
    train_loader = torch.utils.data.DataLoader(Dataset_Train, batch_size=25, shuffle=True, num_workers=0)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            real_data = torch.Tensor(data).cuda().requires_grad_()
