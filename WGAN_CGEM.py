...
def train_D2(): # GAN
    ...
    d_r_score = D( real_data.to(devices) )
    d_r_standard = torch.ones(d_minibatch_size,1)
    d_r_loss = criterion( d_r_score, d_r_standard.to(devices))  # ones = true
    d_r_loss.backward()
    ...
    d_f_loss = criterion( d_f_score, d_f_standard.to(devices))  # zeros = fake
    d_f_loss.backward()
...
def train_G2():
    ...
    fake_data = G( noise )
    g_score = D( fake_data )
    g_standard = torch.zeros(g_minibatch_size,1)
    g_loss = criterion( g_score, g_standard.to(devices) )  # we want to fool, so pretend it's all genuine
    g_loss.backward()
    return g_loss
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for i in range(d_num):
            ...
            real_data = real_data_t.to(devices).requires_grad_()
    ...
    F_Sum, F_x_Mean, F_z_Mean, F_t_Mean, F_x_Std, F_z_Std, F_t_Std = check_fake_data(3000)
    logger.info('Fake data checked!')
    
    # --- Real data check
    R_Sum, R_x_Mean, R_z_Mean, R_t_Mean, R_x_Std, R_z_Std, R_t_Std = load_check_real_data( datafile , 3000)
