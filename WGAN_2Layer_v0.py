...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Sequential(
                    nn.Conv3d(8,16,4,2,0), 
                    nn.LeakyReLU(),
                    nn.MaxPool3d(kernel_size=(2,2,2))
                    )
        '''
        self.conv3 = nn.Sequential(
                    nn.Conv3d(32,48,5,2,0), #20->8
                    nn.LeakyReLU()
                    )
        self.conv4 = nn.Sequential(
                    nn.Conv3d(48,64,5,2,0), #8->2
                    nn.LeakyReLU()
                    )
        self.conv5 = nn.Sequential(
                    nn.Conv3d(64,128,3,1,0), #3->1
                    nn.LeakyReLU()
                    )
        ...
        '''
        self.out = nn.Linear(16*7*7*7,1)
        T_init = torch.randn(16*7*7*7, 64*16) # B=128, C=16
        self.T = nn.Parameter(T_init, requires_grad=True)
        self.out_minidisc = nn.Linear(16*7*7*7 + 64, 1)
...
class Generator(nn.Module):
    def __init__(self, noise_dim):
        ...
        self.linear = nn.Sequential(
                    ...
                    nn.ReLU(),
                    #nn.BatchNorm1d(),
                    nn.Linear(2048, 7*7*7*16),
                    nn.ReLU(),
                    #nn.BatchNorm1d(4*4*4*24)
                    )
        self.inconv1 = nn.Sequential(
                    nn.ConvTranspose3d(16,8,6,4,0),
                    nn.ReLU(),
                    )
        self.inconv2 = nn.Sequential(
                    nn.ConvTranspose3d(8,1,6,4,0),
                    nn.ReLU(),
                    )
        '''
        self.inconv3 = nn.Sequential(
                    nn.ConvTranspose3d(48,32,5,2,0),
                    nn.ReLU(),
                    )
        self.inconv4 = nn.Sequential(
                    nn.ConvTranspose3d(32,16,5,2,0),
                    nn.ReLU(),
                    )
        self.inconv5 = nn.Sequential(
                    nn.ConvTranspose3d(16,1,4,2,0),
                    nn.ReLU(),
                    )
...
def train_G2():
    ...
    g_loss = criterion( g_score, g_standard.to(device) )  # we want to fool, so pretend it's all genuine
    g_loss.backward()
    return g_loss
...
if __name__ == '__main__':
    ...
    d_learning_rate = 4e-5
    g_learning_rate = 5e-6
    ...
    for epoch in range(epochs):
        ...
        for i in range(d_num):
            ...
            real_data = real_data_t.to(device).requires_grad_()
